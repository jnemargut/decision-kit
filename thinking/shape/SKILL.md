...
- `$ARGUMENTS` starts with `/autodecide` (direct invocation modifier — the user typed `/shape /autodecide [project]`)

In the second case, strip `/autodecide` from the args before treating the rest as the user's project.
...
- Order doesn't matter: `/shape /autodecide /overdecide [project]` and `/shape /overdecide /autodecide [project]` are equivalent

Strip all leading modifier tokens from `$ARGUMENTS` before treating the rest as the user's project.
