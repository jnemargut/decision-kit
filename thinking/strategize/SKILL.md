...
- `$ARGUMENTS` starts with `/autodecide` (direct invocation modifier — the user typed `/strategize /autodecide [topic]`)

In the second case, strip `/autodecide` from the args before treating the rest as the user's situation.
...
- Order doesn't matter: `/strategize /autodecide /overdecide [topic]` and `/strategize /overdecide /autodecide [topic]` are equivalent

Strip all leading modifier tokens from `$ARGUMENTS` before treating the rest as the user's situation.
